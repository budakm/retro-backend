    ...
    stage('Deploy into 18.132.36.78 / 10.10.1.24') { 
        sshagent(credentials : ['jenkins-deployer']) {
            withCredentials([usernamePassword( credentialsId: 'postgres-db-credentials', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
                ...
                sh "ssh -o StrictHostKeyChecking=no jenkins@10.10.1.24 \"docker stop ${env.JOB_BASE_NAME} && docker rm ${env.JOB_BASE_NAME}\" || true"
                sh "ssh -o StrictHostKeyChecking=no jenkins@10.10.1.24 \"docker run -d -e \"DB_USER=${USERNAME}\" -e \"DB_PASSWORD=${PASSWORD}\" -e \"DB_HOST=10.10.1.179\" -e \"DB_NAME=wmr\" --name=${env.JOB_BASE_NAME} -p 46991:46991 -p 46992:46992 budakdigital/wmr:${env.JOB_BASE_NAME}-${env.BUILD_NUMBER}\""
            }
        }
    }
