    ...
    stage('Deploy into 18.132.36.78 / 10.10.1.125') { 
        sshagent(credentials : ['jenkins-deployer']) {
            withCredentials([usernamePassword( credentialsId: 'postgres-db-credentials', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
                ...
                sh "ssh -o StrictHostKeyChecking=no jenkins@10.10.1.125 \"docker run -d -e \"DB_USER=${USERNAME}\" -e \"DB_PASSWORD=${PASSWORD}\" -e \"DB_HOST=10.10.1.179\" -e \"DB_NAME=wmr\" --name=${env.JOB_BASE_NAME} -p 46991:46991 -p 46992:46992 --restart unless-stopped budakdigital/wmr:${env.JOB_BASE_NAME}-${env.BUILD_NUMBER}\""
            }
        }
    }
